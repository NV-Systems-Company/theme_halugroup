...
if (!nv_function_exists('nv_block_image_slider')) {
    ...
    {
        global $global_config, $site_mods, $client_info;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.image_slider.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.image_slider.tpl')) {
            $block_theme = $global_config['site_theme'];
        } else {
            $block_theme = 'default';
        }

        $xtpl = new XTemplate('global.image_slider.tpl', NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks');
    ...
    function nv_block_image_slider($block_config)
    {
        ...
        foreach ($block_config['image'] as $index => $image) {
            ...
            if (nv_is_url($image['link'])) {
                $xtpl->parse('main.loop.image.link');
            } else {
                $xtpl->parse('main.loop.image.nolink');
            }
